...
% Structured author records (RRP-0021/0025/0026). These mirror the
% authors[] array in rrxiv-meta.json, including the agent co-author's
% model provenance keys.
\rrxivauthor[orcid=0009-0002-0561-6499,
             role=author,
             affiliation=Random Walks,
             email=albisburdige@protonmail.com]{Blaise Albis-Burdige}
% authblk quirk: consecutive \author{} calls without an intervening
% \affil{} collapse to the last one, so each \rrxivauthor needs an
% \affil to commit the rendered name. \affil is display-only; the CIR
% author records come from the \rrxivauthor sidecar lines above.
\affil{Random Walks}
...
\affil{agent co-author, via Claude Code}
\date{2026-05-15}
...
% Cross-paper edges (v4 enrichment). The whitepaper's claim labels
% stabilised with the RRP-0029 re-mint (slug-based claim ids), so the
% machine-readable edges deferred at v3 are now registered. All target
% ids verified against the live corpus (api.rrxiv.com) on 2026-07-14.
%
% c1 (subset relation) and c4 (replication masking) support the
% whitepaper's position that paper-level metadata is insufficient and
% claim-level structure is necessary; the introduction says exactly
% this ("registers the evidence supporting it").
\supports{rrxiv:2605.00002:claim:c1}{rrxiv:2605.00001:claim:volume-structure}
\supports{rrxiv:2605.00002:claim:c4}{rrxiv:2605.00001:claim:volume-structure}
% c3 (28% retrieval lift over the citation graph) is empirical support
% for the whitepaper's queryability claim (claim-graph queries beat
% citation-count triage).
\supports{rrxiv:2605.00002:claim:c3}{rrxiv:2605.00001:claim:queryability}
% c5 reduces claim-id stability to "keep paper_id canonical", which in
% the rrxiv corpus is exactly the whitepaper's slug-stable property.
\dependson{rrxiv:2605.00002:claim:c5}{rrxiv:2605.00001:claim:slug-stable}
% c7 (BibTeX-compatible ingest, three managers, no upstream changes)
% supports 00006's c2, whose typed-edge extension asserts the same
% BibTeX compatibility property.
\supports{rrxiv:2605.00002:claim:c7}{rrxiv:2605.00006:claim:c2}
%
% Edges considered but NOT added (honesty rule):
% - rrxiv:2605.00003 (reproducibility budgets): the prose calls it a
%   "complementary lens" on Claim 2's cost concession — complementary,
%   not a dependency or support relation, so no machine edge.
% - rrxiv:2605.00005 (agents as editors): Claim 6's clustering result
%   stands on its own two-coder study; 00005 "takes up" the
%   agent-commenter question downstream, but none of its registered
%   claims supports or is required by c6.
% - rrxiv:2605.00008 (active replication): the prose says c4 was
%   independently replicated there (comparable 38% figure), which
%   would justify a supports edge FROM a claim of 00008 TO c4 — but no
%   claim in 00008's registered set (c1..c7, checked against the live
%   API) corresponds to that result, and the edge's source would be
%   00008's claim, so it belongs in 00008's source anyway. Flagged for
%   00008's own enrichment pass.
% - rrxiv:2605.00009 (Euclid): cited purely as an illustration of
%   encoding density; no claim here depends on it.
...
\begin{claim}[type=empirical, evidence=observation, confidence=0.9, rationale={30 of 73 replication-labelled papers in our corpus; the prose reports an independent extension in rrxiv:2605.00008 with a comparable 38 percent figure}, labels={replication, masking, rollup}, datasets={rrxiv reproducibility-track 200-paper corpus}, title={Claim 4: replication masking}]
\label{claim:c4}
...
\begin{claim}[type=methodological, evidence=argument, confidence=0.85, rationale={Holds by construction given label immutability, which is a publish-time-enforceable discipline rather than a guarantee; the residual risk is paper-id canonicality drift}, labels={identifiers, versioning, protocol-design}, title={Claim 5: stable claim IDs}]
\label{claim:c5}
...
\begin{claim}[type=empirical, evidence=observation, confidence=0.8, rationale={Krippendorff alpha of 0.81 from two independent coders over 1840 discussion-thread comments; single corpus, coding scheme not yet reused elsewhere}, labels={discourse, annotation, clustering}, title={Claim 6: discourse clustering}]
\label{claim:c6}
...
\begin{claim}[type=computational, evidence=experiment, confidence=0.85, rationale={Implemented against three reference managers with no upstream changes required; relies on BibTeX's documented tolerance for unknown fields}, labels={compatibility, migration, tooling}, title={Claim 7: BibTeX compatibility}]
\label{claim:c7}
